...
func (g *DataSync) Process(bloxConfig string) error {
	...
	staticDir, err := g.cfg.GetString("build_dir")
	if err != nil {
	...
	err = filepath.Walk(staticDir,
		func(path string, info os.FileInfo, err error) error {
			...
			if !info.IsDir() {
				...
				bucketURL := os.Getenv("DATA_BUCKET")
				if bucketURL == "" {

					g.logger.Error("Bucket Missing", "bucketURL", bucketURL)
					return errors.New("no DATA_BUCKET environment variable set")
				}
